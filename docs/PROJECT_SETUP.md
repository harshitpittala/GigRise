...
### 10.1 Hosting Platform — v1.1 addition, resolves `CROSS_DOCUMENT_REVIEW.md` §2.21; v1.2 amendment per `DECISIONS.md` ADR-019
**`apps/web` and `apps/admin` (Next.js): Netlify** (amended from the original Vercel recommendation — see `DECISIONS.md` ADR-019). Both apps are connected to the same monorepo via Netlify's native pnpm-workspace/monorepo support (per-site base directory, root-level install), with `@netlify/plugin-nextjs` (Netlify's official Next.js runtime) providing App Router/server-component support. Netlify's deploy-preview-per-PR and instant-rollback model satisfies §10's blue/green-equivalent deployment behavior for the frontend apps, the same way Vercel's would have.
**`apps/api` (FastAPI): a container-based platform** (Render, Fly.io, or Railway are all suitable — the specific choice is a founder/team-familiarity call, not an architectural one, since all three support the same container-image-plus-health-check deployment model §10 already assumes) — recommended default: **Render**, for its straightforward managed-Postgres-adjacent tooling and native GitHub Actions integration, though this is explicitly a recommendation pending founder confirmation, not a locked-in decision the way the frontend hosting platform is.
This resolves the "no concrete hosting platform named" gap identified in the cross-document review — the blue/green/canary strategy below is no longer describing a hypothetical platform, it's describing Netlify's deploy-preview/rollback model (frontend) and Render's (or the chosen alternative's) rolling-deploy-with-health-check model (backend).
